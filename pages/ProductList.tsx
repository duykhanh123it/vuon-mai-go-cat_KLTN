...
const splitSpecsFromDescription = (desc?: string) => {
  ...
  const looksLikeSpecs = first.includes("·") || /Cao\s*~|Tán\s*~|Hoành\s*\d+/i.test(first);
...
const getInternalStatus = (p: any) => {
  ...
  if (hasRent && hasSell) return { label: "Thuê + Bán", cls: "bg-emerald-600 text-white" };
...
const ProductCard: React.FC<ProductCardProps> = ({ p, onOpenDetail, onContact }) => {
  ...
  const hasPriceToShow = (publicRent != null && publicRent > 0) || (publicSell != null && publicSell > 0);

  let priceLine = "Liên hệ báo giá";
  if (hasPriceToShow) {
    if (publicRent != null && publicRent > 0 && publicSell != null && publicSell > 0) {
      ...
      <div className="p-5 flex flex-col flex-1">
        <div className="flex items-start justify-between gap-2 mb-1">
          ...
          {internalStatus.label !== "Liên hệ" && (
            <span className={`shrink-0 text-[10px] font-bold px-2 py-1 rounded-full ${internalStatus.cls}`}>
        ...
        <div className="mt-auto flex items-end justify-between gap-4">
          ...
          <div className="min-w-0">
            ...
            <p className={`text-lg font-bold ${hasPriceToShow ? "text-slate-800" : "text-red-600"} line-clamp-2`}>
              {priceLine}
            </p>
          </div>
...
const safeReadCache = (type: ProductsType): CacheShape | null => {
  try {
    ...
    const items = Array.isArray(parsed?.items) ? (parsed.items as Product[]) : null;
...
const saveCache = (type: ProductsType, mapped: Product[], imgVersion: string, dataVersion: string) => {
  try {
...
const ProductList: React.FC<ProductListProps> = ({
  ...
  const apiType: ProductsType =
    filterType === "Mai Bonsai" ? "BS" : filterType === "Mai Tàng" ? "T" : "All";
  ...
  const [totalsByType, setTotalsByType] = useState<{ BS: number; T: number }>({ BS: 0, T: 0 });
  const totalAll = (totalsByType.BS || 0) + (totalsByType.T || 0);

  // init từ cache All để vào là có data ngay
  const [products, setProducts] = useState<Product[]>(() => safeReadCache("All")?.items ?? []);
  ...
  const [cacheVersions, setCacheVersions] = useState<{ imgVersion: string; dataVersion: string }>(() => {
    const c = safeReadCache("All");
    return { imgVersion: String(c?.imgVersion || ""), dataVersion: String(c?.dataVersion || "") };
  });
   ...
   */
  const mapToProduct = (sp: any, imgVersion: string, forcedType: ProductsType): Product => {
    const id = String(sp?.maCay || sp?.id || "").trim();
    const name = id;

    const category = forcedType === "BS" ? "Mai Bonsai" : forcedType === "T" ? "Mai Tàng" : "Khác";

    // specs (để hiển thị dòng thông số trên card)
    const parts: string[] = [];
    if (sp?.cao_m != null && sp?.cao_m !== "") parts.push(`Cao ~ ${sp.cao_m}m`);
    if (sp?.ngang_m != null && sp?.ngang_m !== "") parts.push(`Tán ~ ${sp.ngang_m}m`);
    if (sp?.hoanh_cm != null && sp?.hoanh_cm !== "") parts.push(`Hoành ${sp.hoanh_cm}cm`);
    if (sp?.chau_m != null && sp?.chau_m !== "") parts.push(`Chậu ~ ${sp.chau_m}m`);
    const specs = parts.length ? parts.join(" · ") : "";

    const note = String(sp?.note || "").trim();

    // ✅ Dùng ". " để splitSpecsFromDescription hoạt động ổn
    const description = specs && note ? `${specs}. ${note}` : specs ? `${specs}.` : note || "";
    ...
    const rentPrice = millionToVnd(sp?.giaThue ?? sp?.GiaThue ?? sp?.GiaThuê ?? sp?.GiaMua ?? null);
    ...
    (p as any).__filterRentPrice = millionToVnd(sp?.giaThueFilter ?? sp?.__giaThueNoiBo ?? null);
    (p as any).__filterSellPrice = millionToVnd(sp?.giaBanFilter ?? sp?.__giaBanNoiBo ?? null);
   ...
   */
  const reloadFromServer = async (aliveRef: () => boolean, type: ProductsType) => {
    const setTotals = (bsTotal: number, tTotal: number) => setTotalsByType({ BS: bsTotal, T: tTotal });
    ...
    if (type === "All") {
      ...
      const mappedBS = bs.items.map((sp: any) => mapToProduct(sp, imgVersion, "BS"));
      const mappedT = t.items.map((sp: any) => mapToProduct(sp, imgVersion, "T"));
    ...
    const mapped = main.items.map((sp: any) => mapToProduct(sp, imgVersion, type));
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        if (c?.items?.length) {
          setProducts(c.items);
          setCacheVersions({ imgVersion: String(c.imgVersion || ""), dataVersion: String(c.dataVersion || "") });
  ...
  useEffect(() => {
    ...
    const getInterval = () => (document.visibilityState === "visible" ? 2000 : 20000);
    ...
    const run = async () => {
      ...
      try {
        ...
        const changed = next.imgVersion !== cur.imgVersion || next.dataVersion !== cur.dataVersion;
  ...
  useEffect(() => {
    setPage(1);
    setProductsPage(1);
  }, [filterType, priceMode, filterPrice, filterHeight, searchTerm, setProductsPage]);
  ...
  return (
    <div className="bg-slate-50 min-h-screen pb-2">
        ...
        {loadingProducts && products.length === 0 && (
          <div className="mb-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 opacity-60">
            {Array.from({ length: 6 }).map((_, i) => (
              <div key={i} className="h-[280px] bg-slate-200 rounded-2xl animate-pulse" />
            ))}
          </div>
        )}

        {productsError && <div className="mb-6 text-center text-red-600 text-sm">Lỗi tải sản phẩm: {productsError}</div>}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[280px_1fr] gap-8 items-start">
          ...
          <aside className="bg-white rounded-2xl p-5 shadow-sm border border-slate-100">
            ...
            >
              ...
              <svg
                className={`h-5 w-5 text-slate-600 transition-transform duration-200 ${isFilterOpen ? "rotate-180" : ""}`}
                viewBox="0 0 20 20"
                fill="currentColor"
                aria-hidden="true"
              >
            ...
            <div className={`${isFilterOpen ? "block" : "hidden"} lg:block`}>
              <div className="space-y-6">
                {/* Sản phẩm */}
                <div>
                  <label className="text-sm text-slate-500 block mb-3">Sản Phẩm</label>
                  <div className="flex flex-col gap-2">
                    {categoryOptions.map((type) => (
                      <button
                        ...
                        onClick={() => {
                          setFilterType(type);
                          // reset filter phụ để tránh “rụng cây” khó hiểu
                          setPriceMode("rent");
                          setFilterPrice("All");
                          setFilterHeight("All");
                          setSearchTerm("");
                        ...
                        className={`text-left px-4 py-2 rounded-xl text-sm font-bold transition-all ${filterType === type
                ...
                {/* Khung Giá (1 khung) + 2 nút Thuê/Bán */}
                <div>
                  <label className="text-sm text-slate-500 block mb-3">Khung giá</label>

                  <div className="grid grid-cols-2 gap-2 mb-3">
                    <button
                      type="button"
                      onClick={() => setPriceMode("rent")}
                      className={`px-4 py-2 rounded-xl text-sm font-bold transition-all ${priceMode === "rent"
                        ? "bg-amber-400 text-amber-950"
                        : "bg-slate-50 hover:bg-slate-100 text-slate-700"
                        }`}
                    >
                    ...
                    <button
                      ...
                      className={`px-4 py-2 rounded-xl text-sm font-bold transition-all ${priceMode === "sell"
                        ? "bg-amber-400 text-amber-950"
                        : "bg-slate-50 hover:bg-slate-100 text-slate-700"
                        }`}
                    >
                  ...
                  <div className="flex flex-col gap-2">
                    {PRICE_OPTIONS.map((item) => (
                      <button
                        ...
                        className={`text-left px-4 py-2 rounded-xl text-sm font-bold transition-all ${filterPrice === item.key
                          ? "bg-amber-400 text-amber-950"
                          : "bg-slate-50 hover:bg-slate-100 text-slate-700"
                          }`}
                        type="button"
          ...
          <div>
            ...
            <div className="mt-12 bg-amber-50 border border-amber-200 rounded-2xl p-6 flex flex-col md:flex-row items-center justify-between gap-4">
              <div className="text-slate-700 text-center md:text-left">
                <p className="font-bold text-lg">Bạn chưa tìm được cây phù hợp?</p>
                <p className="text-sm text-slate-600 mt-1">🌼 Nhà vườn còn nhiều cây chưa đăng đủ thông tin.</p>
                <p className="text-sm text-slate-600 mt-1">👉 Gọi ngay để được dẫn xem cây đúng ngân sách & không gian của bạn.</p>
              </div>
              ...
              <div className="mt-8 sm:mt-9 mb-6 flex items-center justify-center">
                <div className="flex items-center bg-white text-slate-800 rounded-full shadow-md px-3 py-1.5 sm:px-4 sm:py-2 border border-slate-200">
                  <button
                    ...
                    disabled={safePage <= 1}
                    className={`w-8 h-8 sm:w-9 sm:h-9 rounded-full grid place-items-center transition text-sm ${safePage <= 1 ? "opacity-40 cursor-not-allowed" : "hover:bg-slate-100"
                      }`}
                  ...
                  <div className="flex items-center gap-1.5 sm:gap-2 mx-2 sm:mx-3 whitespace-nowrap">
                    <span className="text-xs text-slate-600 hidden sm:inline">Trang</span>
                    ...
                    <input
                      ...
                      onBlur={() => {
                        ...
                        const next = Math.min(Math.max(1, Math.trunc(n)), totalPages);
                        setPage(next);
                      }}
                  ...
                  <button
                    ...
                    className={`w-8 h-8 sm:w-9 sm:h-9 rounded-full grid place-items-center transition text-sm ${safePage >= totalPages ? "opacity-40 cursor-not-allowed" : "hover:bg-slate-100"
                      }`}
      ...
      {contactOpen && (
        <div className="fixed inset-0 z-[80] bg-black/40 flex items-end md:hidden" onClick={() => setContactOpen(false)}>
          <div className="w-full bg-white rounded-t-3xl p-4 pb-6 shadow-2xl" onClick={(e) => e.stopPropagation()}>
